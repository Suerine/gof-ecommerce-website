...
function Navbar() {
  ...
  useEffect(() => {
   if (!query) {
     setResults([])
     setShowDropdown(false) // ✅ Also hide dropdown when query is cleared
     return
   }

   const fetchResults = async () => {
     try {
       const res = await API.get(`/api/products/search?search=${query}`) // ✅ Use API instance
       setResults(res.data.products?.slice(0, 5) || []) // ✅ Fallback to empty array
       ...
       <div className="flex items-center gap-4 sm:gap-5 md:gap-6 ml-auto">
        ...
        <div className="relative hidden md:block">
         ...
         {showDropdown && results.length > 0 && (
           <div className="absolute top-10 left-0 w-full bg-white border border-gray-300 rounded-lg shadow-lg z-50">
             {results.map((product) => (
               <Link
                 key={product._id}
         ...
         <Link to="/wishlist" className="flex items-center relative hover:text-amber-300">
        ...
        >
          <Link to="/cart" className="flex items-center relative hover:text-amber-300">
        ...
        <button onClick={handleLoginClick} className="hidden md:flex items-center hover:text-amber-300">
